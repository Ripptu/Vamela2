...
export const Header = ({ translate, titleComponent }: any) => {
  return (
    <motion.div
      ...
      className="div max-w-5xl mx-auto text-center mb-8 md:mb-16 relative z-50"
    >
      {titleComponent}
    </motion.div>
  );
...
const Iphone15Pro = ({ rotateX, scale, translate, children }: IphoneProps) => {
    return (
        <motion.div
            ...
            className="relative w-[320px] md:w-[380px] h-[650px] md:h-[780px] flex flex-col items-center select-none"
        >
            {/* ==================== PHONE BODY (Titanium Frame) ==================== */}
            <div 
                className="relative w-full h-full bg-[#1c1c1e] rounded-[55px] shadow-[0_50px_100px_rgba(0,0,0,0.5),inset_0_0_0_2px_rgba(255,255,255,0.1)] p-3 md:p-4 z-20"
            ...
            >
                ...
                {/* Inner Black Bezel */}
                <div className="relative w-full h-full bg-black rounded-[48px] overflow-hidden border-[6px] border-black shadow-inner">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-tr from-white/[0.05] via-transparent to-white/[0.02] z-40 pointer-events-none rounded-[48px]" />

                    {/* Actual Content */}
                    <div className="w-full h-full relative z-30 bg-[#0b141a] overflow-hidden rounded-[42px]">
                        {children}
                    </div>
                </div>

                {/* Outer Frame Shine (Titanium Look) */}
                <div className="absolute inset-0 rounded-[55px] ring-1 ring-white/10 pointer-events-none z-50 shadow-[inset_0_0_10px_rgba(0,0,0,0.8)]" />
            </div>

            {/* Thickness/Depth of the phone (Sides) */}
            <div 
                className="absolute inset-0 bg-[#2a2a2a] rounded-[55px]" 
                style={{ 
            ...
            />
             {/* Side connection to create volume */}
             {/* Using pseudo 3D via multiple layers or just CSS borders for thickness implies simplified approach for performance. 
                 The motion.div handles the rotation, giving the feel. */}
