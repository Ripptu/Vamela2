...
const rowVariants = {
  ...
  animate: () => ({
    ...
    scale: 1,
    transition: {
      delay: Math.random() + 0.5, // Reduced delay for faster initial load feeling
      duration: 1.4,
...
export const DraggableContainer: React.FC<DraggableContainerProps> = ({
  ...
  useEffect(() => {
    ...
    const handleWheelScroll = (event: WheelEvent) => {
      if (!isDragging) {
        // Prevent default browser scroll only inside this container if needed
    ...
    // Attach listener to element instead of window to avoid hijacking page scroll completely
  ...
  return (
    <GridVariantContext.Provider value={variant}>
      <div className={cn("h-[600px] w-full overflow-hidden relative touch-none", className)}>
        <motion.div
...
export const GridBody = memo<GridBodyProps>(
  ({
    children,
    className,
  }) => {
